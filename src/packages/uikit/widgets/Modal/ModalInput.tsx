...
import { formatBigInt } from "packages/utils/formatBalance";
import { trimTrailZero } from "packages/utils/trimTrailZero";
    ...
    <div style={{ position: "relative" }}>
      <StyledTokenInput isWarning={isBalanceZero}>
        <Flex justifyContent="space-between" pl="16px">
          ...
          <Text fontSize="14px">{`Balance: ${displayBalance(max as `${number}`)}`}</Text>
        </Flex>
          ...
          {maxAmount?.isGreaterThan(0) && (
            <>
              <Button
                ...
                style={{ textTransform: "uppercase" }}>{t("Max")}</Button>
            </>
          )}
        </Flex>
      </StyledTokenInput>
