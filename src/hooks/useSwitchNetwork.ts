...
import { useToast } from 'packages/uikit'
...
import { CHAIN_QUERY_NAME, ChainId } from 'config/chains'
...
export function useSwitchNetworkLocal() {
  ...
  return useCallback(
    (chainId: number) => {
      ...
      replaceBrowserHistory('chain', chainId === ChainId.POLYGON ? null : CHAIN_QUERY_NAME[chainId])
    },
    [setSessionChainId],
  )
}
